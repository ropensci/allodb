...
test_that("new_equations() can be subsetted by taxa", {
  ...
  cols <- c("equation_id", "equation_taxa", "equation_allometry",
            "independent_variable", "dependent_variable", "koppen",
            "dbh_min_cm", "dbh_max_cm", "sample_size",
            "dbh_units_original", "output_units_original")
  expect_is(subset_taxa, "data.frame")
  expect_true(all(cols %in% colnames(new_equations())))
  expect_true(all(grepl("Quercus|Acer", subset_taxa$equation_taxa)))
})
...
test_that("equations can be added to the equation dataframe adn then used in the get_biomass function", {
  all_eqtab <- new_equations(new_taxa = c("Quercus ilex", "Castanea sativa"),
  ...
  agb1 <- get_biomass(dbh = 20, genus = "Quercus", coords = c(-78, 40), new_eqtable = all_eqtab)
  agb2 <- get_biomass(dbh = 20, genus = "Quercus", coords = c(-78, 40), new_eqtable = new_eq)
