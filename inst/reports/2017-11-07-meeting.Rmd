...
The goal of this report is to prepare for a meeting -- with Ervan, Krista and Erika -- about compiling allometric equation for ForestGEO sites.

# Meeting goals:

1. Both for tropical and temperate forests, compile tables with variables `<category>` and `equation` -- where `<category>` is a category such as `site, that is easier and quicker to get allometries for;
...
* genus-level allometries are indeed possible
...
For Tropical sites Ervan provided useful tables and code.
...
Ervan, in (1) you mean general and in (2) you mean taxonomic-genus-level?
