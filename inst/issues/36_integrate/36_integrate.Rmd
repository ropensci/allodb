...
In this section I first show that we can programatically replace the parameters a-d from `equations$equation`. However, we have to be careful with potential mismatches. Second I show that there are other parameters, such as `DBA`, `BBL`, and `BFT`, that don't seem to have a corresponding column in the `equations` dataset, so I don't know yet how to replace them. Erika, can you guide me?
...
* 2. These are all the unique equations, showing that -- apart from a-d -- there are other parameters that we need to replace but I don't know yet how.
