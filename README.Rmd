...
_allodb_ was conceived as a framework to standardize and simplify the biomass estimation process across globally distributed extratropical forests. 
...
The resampled values are then used to fit the following linear model: following nonlinear model: $AGB = a \cdot dbh ^ b + e$, with i.i.d. $e \sim \mathcal{N}(0, sigma^2)$. The parameters (_a_, _b_, and _sigma_) are returned by the `est_params()` function.

The resampled values (dots) and new fitted equation (red dotted line) can be visualised with the `illustrate_allodb()` function. 

```{r est-params-acer, eval = FALSE, fig.height=4, fig.width=10}
