...
#' `new_equations()` function. Default is the compiled \pkg{allodb}
#' equation table.
#' @param wna a numeric vector, this parameter is used in the
#' `weight_allom()` function to determine
#' the dbh-related and sample-size related weights attributed to equations
#' without a specified dbh range or sample size, respectively.
#' Default is 0.1
#' @param w95 a numeric vector, this parameter is used in
#' the `weight_allom()` function
#' to determine the value at which the
#' sample-size-related weight reaches 95% of its
